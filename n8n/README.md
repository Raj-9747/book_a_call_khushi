...
3. In Fireflies' webhook settings, point their "Transcription completed" webhook at your deployed `fireflies-webhook` function URL, using the same secret as step 2.
4. **Verify against Fireflies' current docs before going live** — `fireflies-webhook`'s signature header name/format and its GraphQL query field names (`summary.short_summary`, `meeting_link`, etc.) were written against their documented API shape, not tested against a live account. Same for the notetaker bot's email address (`fred@fireflies.ai` at time of writing) hardcoded in `create-booking-event.json`'s Calendar-event node.
